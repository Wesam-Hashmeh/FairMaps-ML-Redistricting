...
number_districts<- 4 #Hard-coded number of Congressional Districts, the minimum is two.
...
node.to.add<-function(add.this.node){
  ...
  for (i in (length(adj.list)):1){
    if(as.numeric(adj.list[[i]][[1]])!=0 && !is.na(adj.list[i])){
...
pick.new.node<- function(){
  ...
  #   if (!is.na(adj.list[i])){
  #   if (adj.list[[i]][[4]]>=temp.max && !is.na(adj.list[[i]][[4]])){
...
adj.list.update<-function(list.new.adj.nodes){
  for (k in 1:(length(list.new.adj.nodes))){ #loop through nodes to check
    ...
    if (It.is.here==0 && !is.na(GLOBALNODESLIST[[list.new.adj.nodes[k]]][[1]])){ #add the new node in the adjacency list
...
final_district<-vector(mode="list", length = number_districts)
temp_list<-list()
temp_list[[length(temp_list)+1]]<-c(0)
temp_list[[length(temp_list)+1]]<-c(0)
temp_list[[length(temp_list)+1]]<-c(0)
...
#for (j in 1:number_districts){
  make.district()
  final_district[1]<<-district.temp
#}
