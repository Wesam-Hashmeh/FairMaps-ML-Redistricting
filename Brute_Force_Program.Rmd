...
add.node<-function(new.adj.node){
  ...
  for (i in (length(GLOBALNODESLIST)):1){
    if(as.numeric(GLOBALNODESLIST[[i]][[1]])==as.numeric(new.adj.node)){
      ...
      temp1[[length(temp1)+1]]<-(as.numeric(GLOBALNODESLIST[[i]][[4]])+1)
...
adj.list.update<-function(list.new.adj.nodes){
  ...
  # for (k in 1:(length(First.adj))){ #loop through nodes to check
  #   It.is.here<-0
  #   
  #   for (i in 1: length(adj.list)){#for each node, check adj.list
  #     print("check")
  #     if(as.numeric(First.adj[k])==(as.numeric(adj.list[[i]][[1]]))){
  #       It.is.here=1
  #       #Update count
  #       adj.list[[i]][[4]]<-as.numeric(adj.list[[i]][[1]])+1
  #     }
