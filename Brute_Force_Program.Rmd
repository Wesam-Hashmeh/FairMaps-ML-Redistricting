...
add.node<-function(new.adj.node){
  #Build the list of lists to add to adjacency list, this is to check that the node given is in the GLOBALNODESLIST
  for (i in (length(GLOBALNODESLIST)):1){
    if(as.numeric(GLOBALNODESLIST[[i]][[1]])==as.numeric(new.adj.node)){
      
      
      #building individual node list to bring into adjacency list
      temp1<-list()
      temp1[[length(temp1)+1]]<-(as.numeric(GLOBALNODESLIST[[i]][[1]]))
      
      temp1[[length(temp1)+1]]<-(as.numeric(GLOBALNODESLIST[[i]][[2]]))
      temp1[[length(temp1)+1]]<-GLOBALNODESLIST[[i]][[3]]
      
      temp1[[length(temp1)+1]]<-(as.numeric(GLOBALNODESLIST[[i]][[4]]))
...
exterior.node.removal<-function(removed.node){
  for (i in (length(GLOBALNODESLIST)):1){
    if ((as.numeric(GLOBALNODESLIST[[i]][[1]]))==as.numeric(removed.node)){
...
node.to.add<-function(add.this.node){
  ...
  for (i in (length(adj.list)):1){
    if((as.numeric(adj.list[[i]][[1]]))==as.numeric(add.this.node)){
      ...
      district.temp[[length(district.temp)+1]]<-temp1 #Add node to adjacency
      temp.pop<=temp.pop+adj.list[[i]][[2]]#Add population, so district will stop growing when it needs to.
      #Once the node is found and added, we can break the loop
      #break
...
adj.list.update<-function(list.of.new.adj.nodes){
  for (k in 1:(length(list.of.new.adj.nodes))){ #loop through nodes to check
    It.is.here<-0
    for (i in 1: length(adj.list)){#for each node, check adj.list
      if(as.numeric(list.of.new.adj.nodes[k])==(as.numeric(adj.list[[i]][[1]]))){
        It.is.here=1
        #Update count
    ...
    if (It.is.here==0){ #add the new node in the adjacency list
      add.node(as.numeric(list.of.new.adj.nodes[k]))
      }
    
    #And remove all traces of node from the exterior list
