...
#Illinois
#illinois_plans<-redist_smc(IL_state_map,8000,compactness = 1,runs = 2 )
...
redist.plot.plans(illinois_plans, draws=1:6, shp=IL_state_map)
```
...
illinois_plans_2020 = illinois_plans_2020 %>%
  mutate(pop_dev = abs(total_pop / get_target(IL_state_map)-1),
         comp=comp_polsby(pl(), IL_state_map,perim_df = county_perims2020),
...
```{r post-run analysis-illinois}

```


Texas saved diagnositic information, just for tuning purposes.
